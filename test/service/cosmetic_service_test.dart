...
void main() {
  ...
  //
  // 이 파일은 처음에 **백엔드를 보지 않고** 추측으로 적었고, 백엔드가 같은
  // 시각에 실제 응답에서 떠낸 골든
  // (`OnO_BACKEND/backend/src/test/resources/cosmetic/golden/`) 과 맞대 봤다.
  // 서로 보고 적으면 같은 오해를 두 번 적을 뿐이라 대조가 의미가 없어서다.
  //
  // 여덟 군데를 추측했고 일곱이 맞았다. 틀린 하나는 아래 1번이고 고쳤다.
  //
  // 1. **틀렸다.** 성공 응답에 `errorCode`, `message` 가 null 로 실려 온다고
  //    봤는데, `CommonResponse` 가 `@JsonInclude(NON_NULL)` 이라 두 키는 아예
  //    빠진다. 파일에서 지웠다. `HttpService` 는 `data` 만 보므로 앱 동작은
  //    어느 쪽이든 같았지만, 계약서가 없는 필드를 있다고 적고 있었다.
  // 2. 슬롯 키가 대문자(`HEAD`)인 것 맞다. `docs/치장 시스템/구현_계획.md:98`
  //    의 소문자 표가 낡았다.
  // 3. `conflictsWith` 가 문자열 배열인 것 맞다. 쉼표로 이은 문자열이 아니다.
  //    다만 **시드에 충돌 조합이 하나도 없어서 실제 응답은 전부 빈 배열이다.**
  //    값이 담긴 모양은 여기 `hat_crown` 한 줄로만 잠겨 있고 서버 골든에는
  //    없다. 충돌 조합을 넣을 때 양쪽을 같이 봐야 한다.
  // 4. `composited`, `baseLayerOrder`, `setNameKo`, `items[].layerOrder`,
  //    `fullBody` 다섯 다 서버가 실제로 내려준다.
  // 5. `imageUrl` 이 앱 번들 경로인 것 맞다. 프레임만 `assets/ProfileFrame/`
  //    아래 SVG 인 것까지 같다.
  // 6. 총 학습으로 열리는 아이템의 `requiredAbility` 는 실제로 null 이다.
  //    `TOTAL_STUDY` 같은 값을 보내지 않는다.
  // 7. `unequippedSlots` 라는 이름도, 그 안에 슬롯 키가 담기는 것도 맞다.
