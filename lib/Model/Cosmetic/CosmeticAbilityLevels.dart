...
class CosmeticAbilityLevels {
  ...
  /// 능력치 넷의 상한. **총 학습과 같은 값이어야 한다.**
  ///
  /// 임계값이 능력치는 `10 × 레벨`, 총 학습은 그 네 배인 `40 × 레벨` 이라
  /// 능력치 넷을 만렙까지 올린 누적이 총 학습 만렙에 필요한 양과 정확히 같다
  /// (`4 × 5N(N-1) = 20N(N-1)`, 모든 N 에서 성립). 둘이 어긋나면 넷을 다
  /// 채워도 총 학습이 만렙에 못 닿거나, 능력치가 먼저 멈춘 채 총 학습만 혼자
  /// 오르는 구간이 생긴다. 그래서 숫자를 다시 적지 않고 묶어 둔다.
  static const int maxAbility = maxTotalStudy;
